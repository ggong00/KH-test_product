...
public class ProductRestController {
    ...
    public ApiResponse<Object> add(@Valid @RequestBody SaveForm saveForm, BindingResult bindingResult){
        log.info("reqMsg={}",saveForm);
        //검증
        ...
        //오브젝트검증
        if(saveForm.getCount() * saveForm.getPrice() > 10_000_000){
    ...
    public ApiResponse<Object> edit(
        ...
        //오브젝트검증
        if(updateForm.getCount() * updateForm.getPrice() > 10_000_000){
            bindingResult.reject(null, "총액(상품수량*단가)이 1000만원을 초과할수 없습니다.");
        }
        if(bindingResult.hasErrors()){
            log.info("bindingResult1={}",bindingResult);
        ...
        //응답메세지
        return ApiResponse.createApiResMsg("01", "성공", productSVC.findBypid(id).get());
    }
